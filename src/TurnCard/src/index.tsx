...
export default Vue.extend({
  ...
  props: {
    ...
    cardstate: {
      ...
      default: () => [],
      validator: function(val) { // 状态校验-卡牌的状态参数必须为1、2、3、4四种类型其中一种
  ...
  render() {
    return (
      <div class="mk-turncard-main" id="mkTurncard">
        {
          this.cards.length && this.cards.map((card, index) => {
            return (
              <div class="mk-turncard-card__box" key={index}>
                <div
                  id={`card${index}`}
                  class={['mk-turncard-card__item', { 'mk-turncard-card__item-turn': this.cardstate[index] > 1 }, { 'mk-turncard-card__item-turn2': this.cardstate[index] > 2 }]}
                  style={{ backgroundImage: this.configbackground(this.cardstate[index], card) }}
