  ...
  render() {
    return (
      <div class="mk-ring-toss">
        {/* 动猫 */}
        {
          this.ringInitAnimateList.length && this.ringInitAnimateList.map((item, key) => {
            return (
              <div
                key={key}
                style={this.initRingStyleList[key]}
                id={item.id}
                class={[`cat${key}`, 'initCat', 'hide', { activeCatInit: item.isInitMove }, { stopCatMove: item.isStop }, { show: item.isShow }]}
              ></div>
            )
          })
        }
        <div class="mk-ring-catch__area">
          {/* 抓猫 */}
          {
            this.ringCatchAnimateList.length && this.ringCatchAnimateList.map((item, key) => {
              return (
                <div
                  key={`${key}0`}
                  class={['initCatchCat', `catchCat${key}`, { active: item.catchActive }]}
          ...
          {/* 逃猫 */}
          {
            this.ringCatchAnimateList.length && this.ringCatchAnimateList.map((item, key) => {
              return (
                <div
                  key={`${key}1`}
                  class={['initEscapeCat', `escapeCat${key}`, { active: item.escapeActive }]}
