...
export default Vue.extend({
  ...
  props: {
    ...
    url: {
      type: String,
      validator (val) {
        return ['https://', 'http://', '//'].some(prefix => val.startsWith(prefix))
      },
      required: true
    },
    // 序列帧的总帧数
  ...
  computed: {
    framesStyle () {
      ...
      const animationStyle = `${this.animationName} ${this.duration / 1000}s steps(1) ${this.pause ? 'paused' : 'running'} ${this.times} ${this.direction}`
      return {
        width: width + 'px',
        height: height + 'px',
  ...
  watch: {
    framsVisible (newVal) {
      ...
      newVal ? framesPlayer?.classList.add('frames-block') : framesPlayer?.classList.remove('frames-block')
    }
  },
    ...
    keyframeGenerator ({ width, height, amount, column }: KeyframsType) {
      ...
      for (let i = 0; i < amount; i++) {
        ...
        if (i >= amount - 2 ) {
          outputHtml += `
            ${percent}% {
              background-position: -${width * whichColumn}px -${height * whichRow}px;
  ...
  render() {
    return (
      <div class="mk-frames-player" style={this.framesStyle} id="framesPlayer"></div>
    )
  }
})
